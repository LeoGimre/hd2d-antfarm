#!/usr/bin/env python3
"""creature_forge.py — draw every creature in the roster.

    python3 game/tools/creature_forge.py            # game/assets/sprites/creatures/
    python3 game/tools/creature_forge.py --sheet    # plus a contact sheet to look at

Ported out of design/proto at tick 60. design/creature_sprites.md decides the
grammar it implements and is the thing to read first; this file is the
implementation and the tuned numbers.

Two things are data and not code, per the pillar that the roster must stay cheap
to grow: which creatures exist and how each is drawn (game/data/creature_art.json,
which names a body plan and only the parameters that creature overrides), and
what type each is (game/data/creatures.json, so a creature's hue and its type
cannot disagree). Per-plan defaults stay here, because they are renderer values
arrived at by opening the output, not content.

New PNGs in game/assets/ need `godot --headless --import` before anything can
load them — without an .import file a texture loads as null, and verify.sh does
not catch it.
"""
import sys, os, json, math, colorsys, zlib, struct
...
# One dict per plan, and the only place these numbers live. An art entry in
# game/data/creature_art.json overrides whichever of them it needs to.
PLAN_DEFAULTS = dict(quadruped=q(), serpent=sp(), avian=av(),
                     blob=bl(), insectoid=ins(), brawler=br())

GAME = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))


def load_roster():
    """(cid, plan, type, features, params) per creature, from game/data.

    Merged over PLAN_DEFAULTS, so an art entry carries only what it changes.
    design/roster.md authors twenty creatures; creatures.json ships the ones
    that have stats. Art for an unshipped creature still draws — the site's
    roster page shows the whole authored roster — which is why the type lives
    here too, with farm/agreements.py checking it against creatures.json rather
    than this file having to guess.
    """
    art = json.load(open(os.path.join(GAME, "data", "creature_art.json")))
    out = []
    for entry in art["creatures"]:
        params = dict(PLAN_DEFAULTS[entry["plan"]])
        params.update(entry.get("params", {}))
        out.append((entry["id"], entry["plan"], entry["type"],
                    entry.get("features", []), params))
    return out
...
OUT = os.environ.get("FORGE_OUT",
                     os.path.join(GAME, "assets", "sprites", "creatures"))


def main():
    os.makedirs(OUT, exist_ok=True)
    made = []
    for (cid, plan, ctype, feats, p) in load_roster():
    ...
    if "--sheet" in sys.argv:
        # A contact sheet is something to look at, not something the game loads,
        # so it does not belong in game/assets — an unimported stray PNG there
        # is exactly the failure this file's header warns about.
        w, h, px = sheet(made, cols=5)
        dest = os.environ.get("FORGE_SHEET", "/tmp/creature_forge_sheet.png")
        print(write_png(dest, w, h, px))
